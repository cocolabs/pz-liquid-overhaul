...
public final class AnimLayer implements IAnimListener {
   ...
   private void updateMaximumTwist(IAnimationVariableSource var1) {
      ...
      if (var2 != null) {
         ...
         for(int var7 = this.m_liveAnimNodes.size() - 1; var7 >= 0; --var7) {
            LiveAnimNode var8 = (LiveAnimNode)this.m_liveAnimNodes.get(var7);
            float var9 = var8.getWeight();
            if (var6 <= 0.0F) {
               break;
            }

            float var10 = PZMath.clamp(var9, 0.0F, var6);
            var6 -= var10;
            float var11 = PZMath.clamp(var8.getSourceNode().m_maxTorsoTwist, 0.0F, 70.0F);
            var5 += var11 * var10;
         }
   ...
   public void updateNodeActiveFlags() {
      ...
      AnimState var11 = this.m_State;
      IAnimatable var12 = this.m_Character;
      int var4;
      if (var11 != null && !var12.getVariableBoolean("AnimLocked")) {
         List var3 = var11.getAnimNodes(var12, this.m_reusableAnimNodes);
         var4 = 0;
      ...
      for(var4 = this.m_liveAnimNodes.size(); var13 < var4; ++var13) {
         LiveAnimNode var14 = (LiveAnimNode)this.m_liveAnimNodes.get(var13);
         if (var14.isNew() && var14.wasActivated()) {
            LiveAnimNode var15 = this.findTransitionToNewNode(var14, false);
            if (var15 != null) {
               AnimTransition var7 = var15.findTransitionTo(var12, var14.getName());
               float var8 = var7.m_speedScale;
               if (var8 == Float.POSITIVE_INFINITY) {
                  var8 = var14.getSpeedScale(this.m_Character);
               }

               AnimationTrack var9 = null;
               if (!StringUtils.isNullOrWhitespace(var7.m_AnimName)) {
                  AnimLayer.StartAnimTrackParameters var10 = AnimLayer.StartAnimTrackParameters.alloc();
                  var10.subLayerBoneWeights = var15.getSubStateBoneWeights();
                  var10.speedScale = var8;
                  var10.deferredBoneName = var15.getDeferredBoneName();
                  var10.deferredBoneAxis = var15.getDeferredBoneAxis();
                  var10.priority = var15.getPriority();
                  var9 = this.startAnimTrack(var7.m_AnimName, var10);
                  var10.release();
                  if (var9 == null) {
                     if (DebugLog.isEnabled(DebugType.Animation)) {
                        DebugLog.Animation.println("  TransitionTo failed to play transition track: %s -> %s -> %s", var15.getName(), var7.m_AnimName, var14.getName());
                     }
                     continue;
                  }

                  if (DebugLog.isEnabled(DebugType.Animation)) {
                     DebugLog.Animation.println("  TransitionTo found: %s -> %s -> %s", var15.getName(), var7.m_AnimName, var14.getName());
                  }
               } else if (DebugLog.isEnabled(DebugType.Animation)) {
                  DebugLog.Animation.println("  TransitionTo found: %s -> <no anim> -> %s", var15.getName(), var14.getName());
               }

               var14.startTransitionIn(var15, var7, var9);
               var15.setTransitionOut(var7);
            }
         }
      }
   ...
   private void applyBlendField(LiveAnimNode var1, float var2, float var3) {
      if (var1.isActive()) {
         ...
         for(int var14 = 0; var14 < var1.m_AnimationTracks.size(); ++var14) {
            Anim2DBlend var15 = (Anim2DBlend)var4.m_2DBlends.get(var14);
            AnimationTrack var16 = (AnimationTrack)var1.m_AnimationTracks.get(var14);
            if (var15 == var7) {
               var16.blendFieldWeight = AnimationPlayer.lerpBlendWeight(var16.blendFieldWeight, var10, 0.15F);
            } else if (var15 == var8) {
      ...
      float var17 = var1.getWeight();

      for(int var18 = 0; var18 < var1.m_AnimationTracks.size(); ++var18) {
         AnimationTrack var19 = (AnimationTrack)var1.m_AnimationTracks.get(var18);
         var19.BlendDelta = var19.blendFieldWeight * var17;
      }
