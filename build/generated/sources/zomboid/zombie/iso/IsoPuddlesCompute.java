...
public final class IsoPuddlesCompute {
   ...
   private static Vector3f floor(Vector3f var0) {
      return new Vector3f((float)Math.floor((double)var0.x), (float)Math.floor((double)var0.y), (float)Math.floor((double)var0.z));
   }

   private static Vector3f fract(Vector3f var0) {
      return new Vector3f(fract(var0.x), fract(var0.y), fract(var0.z));
   }
   ...
   private static float FuncHash(Vector3f var0) {
      Vector3f var1 = new Vector3f(var0.dot(HashVector31), var0.dot(HashVector32), 0.0F);
   ...
   private static float FuncNoise(Vector3f var0) {
      ...
      Vector3f var3 = new Vector3f(var2.x * var2.x * (4.5F - 3.5F * var2.x), var2.y * var2.y * (4.5F - 3.5F * var2.y), var2.z * var2.z * (4.5F - 3.5F * var2.z));
      float var4 = mix(FuncHash(var1), FuncHash((new Vector3f(var1)).add(add_xyy)), var3.x);
      float var5 = mix(FuncHash((new Vector3f(var1)).add(add_yxy)), FuncHash((new Vector3f(var1)).add(add_xxy)), var3.x);
      float var6 = mix(FuncHash((new Vector3f(var1)).add(add_yyx)), FuncHash((new Vector3f(var1)).add(add_xyx)), var3.x);
      float var7 = mix(FuncHash((new Vector3f(var1)).add(add_yxx)), FuncHash((new Vector3f(var1)).add(add_xxx)), var3.x);
   ...
   private static float PerlinNoise(Vector3f var0) {
      if (hd_quality) {
         ...
         var1 = (float)((double)var1 * Math.min(1.0D, 2.0D * (double)FuncNoise((new Vector3f(var0)).mul(0.02F)) * Math.min(1.0D, 1.0D * (double)FuncNoise((new Vector3f(var0)).mul(0.1F)))));
         return var1;
      } else {
   ...
   private static float getPuddles(Vector2f var0) {
      ...
      var4 = (float)((double)var4 + (double)var1 * Math.sin(((double)var0.x * 1.0D + (double)var0.y * 2.0D) * (double)Pi * 1.0D) * Math.cos(((double)var0.x * 1.0D + (double)var0.y * 2.0D) * (double)Pi * 1.0D) * 2.0D);
      var4 = (float)((double)var4 + (double)var2 * Math.sin(((double)var0.x * 1.0D - (double)var0.y * 2.0D) * (double)Pi * 1.0D) * Math.cos(((double)var0.x * 1.0D - (double)var0.y * 2.0D) * (double)Pi * 1.0D) * 2.0D);
      var4 = (float)((double)var4 + (double)var3 * 0.3D);
      float var5 = PerlinNoise(new Vector3f(var0.x * 1.0F, 0.0F, var0.y * 2.0F));
      float var6 = Math.min(0.7F, var4 * var5);
      var5 = Math.min(0.7F, PerlinNoise(new Vector3f(var0.x * 0.7F, 1.0F, var0.y * 0.7F)));
      return var6 + var5;
   }
   ...
   public static float computePuddle(IsoGridSquare var0) {
      ...
      if (!Core.getInstance().getUseShaders()) {
         ...
         if (Core.getInstance().getPerfPuddles() > 0 && var0.z > 0) {
            ...
            Vector4f var2 = var1.getShaderOffset();
            var2.x -= 90000.0F;
            var2.y -= 640000.0F;
            ...
            Vector2f var9 = new Vector2f((var5 * var2.z + var2.x) * 8.0E-4F + (float)var0.z * 7.0F, (var6 * var2.w + var2.y) * 8.0E-4F + (float)var0.z * 7.0F);
