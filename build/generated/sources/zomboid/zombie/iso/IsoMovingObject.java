...
public class IsoMovingObject extends IsoObject implements Mover {
   ...
   public void postupdate() {
      ...
      if (GameClient.bClient && this.isZombie()) {
         ...
         var11 = this.bspeed;
         var11 *= GameTime.getInstance().getServerMultiplier();
         if (tempo.getLength() > 2.0F) {
            var11 = tempo.getLength();
         }

         if (!(var11 > 0.001F) || tempo.x == 0.0F && tempo.y == 0.0F) {
            this.bx = this.x;
            this.by = this.y;
         } else {
            if (var11 < tempo.getLength()) {
               tempo.setLength(var11);
            }
         ...
         if (!this.destroyed) {
            ...
            if (!GameClient.bClient && this.isZombie() && (int)this.z == 0 && this.getCurrentBuilding() == null && !this.isInLoadedArea((int)this.nx, (int)this.ny) && (((IsoZombie)this).getCurrentState() == PathFindState.instance() || ((IsoZombie)this).getCurrentState() == WalkTowardState.instance())) {
               ...
               if (this instanceof IsoGameCharacter && !this.isOnFloor() && ((IsoGameCharacter)this).getVehicle() == null && this.isCollidable() && (!(this instanceof IsoPlayer) || !((IsoPlayer)this).isNoClip())) {
                  Vector2f var10 = PolygonalMap2.instance.resolveCollision((IsoGameCharacter)this, this.nx, this.ny, IsoMovingObject.L_postUpdate.vector2f);
                  if (var10.x != this.nx || var10.y != this.ny) {
                     this.nx = var10.x;
                     this.ny = var10.y;
               ...
               if (var13 != null && !var13.isCurrentState(CollideWithWallState.instance()) && !this.collidedN && !this.collidedS && !this.collidedW && !this.collidedE) {
               ...
               if (GameClient.bClient) {
                  ...
                  tempo.x = this.x - this.bx;
                  tempo.y = this.y - this.by;
                  float var9 = this.bspeed;
                  if (this.isZombie()) {
                     var9 *= GameTime.getInstance().getServerMultiplier();
                  }

                  if (tempo.getLength() > 2.0F) {
                     var9 = tempo.getLength();
                  }

                  if (var9 > 0.001F && (tempo.x != 0.0F || tempo.y != 0.0F)) {
                     if (var9 < tempo.getLength()) {
                        tempo.setLength(var9);
                     }
               ...
               if (this.current == null) {
                  for(int var14 = (int)this.z; var14 >= 0; --var14) {
                     this.current = this.getCell().getGridSquare((int)this.x, (int)this.y, var14);
                     if (this.current != null) {
                        break;
                     }
                  }
         ...
         if (this.isPushableForSeparate()) {
            ...
            for(int var3 = -1; var3 <= 1; ++var3) {
               for(int var4 = -1; var4 <= 1; ++var4) {
                  IsoGridSquare var5 = this.getCell().getGridSquare(this.current.getX() + var3, this.current.getY() + var4, (int)this.z);
                  if (var5 != null) {
                     for(int var6 = 0; var6 < var5.getMovingObjects().size(); ++var6) {
                        IsoMovingObject var7 = (IsoMovingObject)var5.getMovingObjects().get(var6);
                        if (var7 != this && var7.isSolidForSeparate() && !(Math.abs(this.z - var7.z) > 0.3F)) {
                           IsoGameCharacter var8 = (IsoGameCharacter)Type.tryCastTo(var7, IsoGameCharacter.class);
                           IsoPlayer var9 = (IsoPlayer)Type.tryCastTo(var7, IsoPlayer.class);
                           float var10 = this.width + var7.width;
                           Vector2 var11 = tempo;
                           var11.x = this.nx - var7.nx;
                           var11.y = this.ny - var7.ny;
                           float var12 = var11.getLength();
                           if (var1 == null || var8 == null && !(var7 instanceof BaseVehicle)) {
                              if (var12 < var10) {
                                 CollisionManager.instance.AddContact(this, var7);
                              }
                           ...
                           if (var8 != null) {
                              if (var12 < var10 + 16.0F && var2 != null && var2.getBumpedChr() != var7 && var2.getBeenSprintingFor() >= 70.0F && WeaponType.getWeaponType((IsoGameCharacter)var2) == WeaponType.spear) {
                              ...
                              if (!(var12 >= var10)) {
                                 if (var2 != null && var2.getVariableFloat("WalkSpeed", 0.0F) > 0.2F && var2.runningTime > 15.0F && var2.getBumpedChr() != var7 && !"charge".equals(var2.getAttackType())) {
                                    if (this.isOnFloor() || var1.getBumpedChr() == null && (System.currentTimeMillis() - var2.getLastBump()) / 100L >= 15L && !var2.isSprinting() || var9 != null && var9.isNPC()) {
                                       var1.bumpNbr = 0;
                                    } else {
                                       var1.actionContext.reportEvent("wasBumped");
                                       ++var1.bumpNbr;
                                       int var13 = 10 - var1.bumpNbr * 2;
                                       ...
                                       if (Rand.Next(var13) == 0 || var1.isSprinting()) {
                                          ...
                                          var1.setVariable("BumpFall", true);
                                       }
                                    }

                                    var1.setLastBump(System.currentTimeMillis());
                                    var1.setBumpedChr(var8);
                                    var1.setBumpType(this.getBumpedType(var8));
                                    boolean var16 = var1.isBehind(var8);
                                    ...
                                    var8.setBumpType(var15);
                                    var8.setHitFromBehind(var16);
                                 }

                                 if (GameServer.bServer || this.distToNearestCamCharacter() < 60.0F) {
                                    if (this.isPushedByForSeparate(var7)) {
                                       var11.setLength((var12 - var10) / 8.0F);
                                       this.nx -= var11.x;
                                       this.ny -= var11.y;
                                    }

                                    this.collideWith(var7);
                                 }
                              }
                           }
                        }
         ...
         if (var1 != null && (this.isOnFloor() || var1.bKnockedDown)) {
            if (!var1.isCurrentState(ClimbOverFenceState.instance()) && !var1.isCurrentState(ClimbThroughWindowState.instance())) {
               if (var1.hasAnimationPlayer() && var1.getAnimationPlayer().isReady()) {
                  ...
                  Vector2 var4 = IsoMovingObject.L_slideAwayFromWalls.vector2.set(var2.x - this.x, var2.y - this.y);
                  var4.normalize();
                  var2.x += var4.x * 0.3F;
                  var2.y += var4.y * 0.3F;
                  float var6;
                  if (var1.bKnockedDown && (var1.isCurrentState(ZombieFallDownState.instance()) || var1.isCurrentState(StaggerBackState.instance()))) {
                     Vector2f var5 = PolygonalMap2.instance.resolveCollision(var1, var2.x, var2.y, IsoMovingObject.L_slideAwayFromWalls.vector2f);
                     if (var5.x != var2.x || var5.y != var2.y) {
                        var6 = GameTime.getInstance().getMultiplier() / 5.0F;
                        this.nx += (var5.x - var2.x) * var6;
                        this.ny += (var5.y - var2.y) * var6;
                        return;
                     }
                  }

                  if ((int)var2.x != this.current.x || (int)var2.y != this.current.y) {
                     IsoGridSquare var7 = this.getCell().getGridSquare((int)var2.x, (int)var2.y, (int)this.z);
                     if (var7 != null) {
                        if (this.current.testCollideAdjacent(this, var7.x - this.current.x, var7.y - this.current.y, 0)) {
                           var6 = GameTime.getInstance().getMultiplier() / 5.0F;
                           if (var7.x < this.current.x) {
                              this.nx += ((float)this.current.x - var2.x) * var6;
                           } else if (var7.x > this.current.x) {
                              this.nx += ((float)var7.x - var2.x) * var6;
                           }

                           if (var7.y < this.current.y) {
                              this.ny += ((float)this.current.y - var2.y) * var6;
                           } else if (var7.y > this.current.y) {
                              this.ny += ((float)var7.y - var2.y) * var6;
                           }
