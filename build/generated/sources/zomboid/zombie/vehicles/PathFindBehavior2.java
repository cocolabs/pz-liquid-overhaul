...
public final class PathFindBehavior2 implements PolygonalMap2.IPathfinder {
   ...
   private static final ObjectPool actualPool = new ObjectPool(() -> {
      return new PathFindBehavior2.DebugPt();
   });
   ...
   public PathFindBehavior2.BehaviorResult update() {
      if (this.chr.getFinder().progress == AStarPathFinder.PathFindProgress.notrunning) {
         PolygonalMap2.PathFindRequest var12 = PolygonalMap2.instance.addRequest(this, this.chr, this.startX, this.startY, this.startZ, this.targetX, this.targetY, this.targetZ);
         ...
         if (var1 != ClimbOverFenceState.instance() && var1 != ClimbThroughWindowState.instance()) {
            if (this.chr.getVehicle() != null) {
               ...
               PolygonalMap2.PathNode var13;
               if (this.pathIndex == this.path.nodes.size() - 2) {
                  var13 = (PolygonalMap2.PathNode)this.path.nodes.get(this.path.nodes.size() - 1);
                  if (IsoUtils.DistanceToSquared(this.chr.x, this.chr.y, var13.x, var13.y) <= 0.0025000002F) {
               ...
               var13 = (PolygonalMap2.PathNode)this.path.nodes.get(this.pathIndex);
               PolygonalMap2.PathNode var4 = (PolygonalMap2.PathNode)this.path.nodes.get(this.pathIndex + 1);
               float var5 = var4.x;
               float var6 = var4.y;
               Vector2 var7 = tempVector2.set(var4.x - this.chr.x, var4.y - this.chr.y);
               var7.normalize();
               this.chr.getDeferredMovement(tempVector2_2);
               float var8 = tempVector2_2.getLength();
               if (var2 != null) {
                  var2.bRunning = false;
               ...
               float var11 = IsoUtils.DistanceTo(var5, var6, this.chr.x, this.chr.y);
               if (var10 >= var11) {
                  var8 *= var11 / var10;
                  ++this.pathIndex;
               }
               ...
               if (var2 != null) {
                  this.checkCrawlingTransition(var13, var4, var11);
               }

               if (var2 == null && var11 >= 0.5F) {
                  if (this.checkDoorHoppableWindow(this.chr.x + var7.x * Math.max(0.5F, var8), this.chr.y + var7.y * Math.max(0.5F, var8), this.chr.z)) {
                     return PathFindBehavior2.BehaviorResult.Failed;
                  }
               ...
               if (var8 <= 0.0F) {
                  ...
                  tempVector2_2.set(var7);
                  tempVector2_2.setLength(var8);
                  ...
                  if (this.isStrafing()) {
                     ...
                     this.chr.faceLocationF(var5, var6);
                  }

                  if (var2 != null) {
                     var2.reqMovement.x = this.chr.getForwardDirection().x;
                     var2.reqMovement.y = this.chr.getForwardDirection().y;
                  }
