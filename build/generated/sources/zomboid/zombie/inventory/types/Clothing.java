...
public class Clothing extends InventoryItem {
   ...
   public void DoTooltip(ObjectTooltip var1, ObjectTooltip.Layout var2) {
      ...
      if (!this.isEquipped() && var1.getCharacter() != null) {
         float var13 = 0.0F;
         float var14 = 0.0F;
         WornItems var15 = var1.getCharacter().getWornItems();

         for(int var16 = 0; var16 < var15.size(); ++var16) {
            WornItem var17 = var15.get(var16);
            if (this.getBodyLocation().equals(var17.getLocation()) || var15.getBodyLocationGroup().isExclusive(this.getBodyLocation(), var17.getLocation())) {
               var13 += ((Clothing)var17.getItem()).getBiteDefense();
               var14 += ((Clothing)var17.getItem()).getScratchDefense();
            }
         }

         float var19 = this.getBiteDefense();
         if (var19 != var13) {
            var3 = var2.addItem();
            if (var19 > 0.0F || var13 > 0.0F) {
               var3.setLabel(Translator.getText("Tooltip_BiteDefense") + ":", 1.0F, 1.0F, 0.8F, 1.0F);
               if (var19 > var13) {
                  var3.setValue((int)var19 + " (+" + (int)(var19 - var13) + ")", 0.0F, 1.0F, 0.0F, 1.0F);
               } else {
                  var3.setValue((int)var19 + " (-" + (int)(var13 - var19) + ")", 1.0F, 0.0F, 0.0F, 1.0F);
               }
            }
         } else if (this.getBiteDefense() != 0.0F) {
            var3 = var2.addItem();
            var3.setLabel(Translator.getText("Tooltip_BiteDefense") + ":", 1.0F, 1.0F, 0.8F, 1.0F);
            var3.setValueRightNoPlus((new Float(this.getBiteDefense())).intValue());
         }

         float var20 = this.getScratchDefense();
         if (var20 != var14) {
            var3 = var2.addItem();
            if (var20 > 0.0F || var14 > 0.0F) {
               var3.setLabel(Translator.getText("Tooltip_ScratchDefense") + ":", 1.0F, 1.0F, 0.8F, 1.0F);
               if (var20 > var14) {
                  var3.setValue((int)var20 + " (+" + (int)(var20 - var14) + ")", 0.0F, 1.0F, 0.0F, 1.0F);
               } else {
                  var3.setValue((int)var20 + " (-" + (int)(var14 - var20) + ")", 1.0F, 0.0F, 0.0F, 1.0F);
               }
            }
         } else if (this.getScratchDefense() != 0.0F) {
            var3 = var2.addItem();
            var3.setLabel(Translator.getText("Tooltip_ScratchDefense") + ":", 1.0F, 1.0F, 0.8F, 1.0F);
            var3.setValueRightNoPlus((new Float(this.getScratchDefense())).intValue());
         }
      } else {
      ...
      if (Core.bDebug && DebugOptions.instance.TooltipInfo.getValue()) {
         ...
         if (this.bloodLevel != 0.0F) {
            ...
            var18 = (int)Math.ceil((double)this.bloodLevel);
            var3.setValueRight(var18, false);
         }

         if (this.dirtyness != 0.0F) {
            var3 = var2.addItem();
            var3.setLabel("DBG: dirtyness:", 1.0F, 1.0F, 0.8F, 1.0F);
            var18 = (int)Math.ceil((double)this.dirtyness);
            var3.setValueRight(var18, false);
         }
         ...
         if (this.wetness != 0.0F) {
            ...
            var18 = (int)Math.ceil((double)this.wetness);
            var3.setValueRight(var18, false);
         }
      }
   ...
   public void save(ByteBuffer var1, boolean var2) throws IOException {
      ...
      if (this.patches != null) {
         var1.putInt(this.patches.size());
         Iterator var3 = this.patches.keySet().iterator();

         while(var3.hasNext()) {
            int var4 = (Integer)var3.next();
            var1.putInt(var4);
            ((Clothing.ClothingPatch)this.patches.get(var4)).save(var1, var2);
         }
      } else {
         var1.putInt(0);
      }
   ...
   public float getDefForPart(BloodBodyPartType var1, boolean var2) {
      if (this.getVisual().getHole(var1) > 0.0F) {
         return 0.0F;
      } else {
         Clothing.ClothingPatch var3 = this.getPatchType(var1);
         float var4 = this.getScratchDefense();
         if (var2) {
            var4 = this.getBiteDefense();
         }

         if (var1 == BloodBodyPartType.Neck && this.getScriptItem().neckProtectionModifier < 1.0F) {
            var4 *= this.getScriptItem().neckProtectionModifier;
         }

         if (var3 != null) {
            int var5 = var3.scratchDefense;
            if (var2) {
               var5 = var3.biteDefense;
            }
