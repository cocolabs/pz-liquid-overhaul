...
public final class ItemContainer {
   ...
   private static final ThreadLocal TL_comparators = ThreadLocal.withInitial(() -> {
      return new ItemContainer.Comparators();
   });
   private static final ThreadLocal TL_itemListPool = ThreadLocal.withInitial(ItemContainer.InventoryItemListPool::new);
   private static final ThreadLocal TL_predicates = ThreadLocal.withInitial(() -> {
      return new ItemContainer.Predicates();
   });
   ...
   private static int getUses(ItemContainer.InventoryItemList var0) {
      ...
      for(int var2 = 0; var2 < var0.size(); ++var2) {
         DrainableComboItem var3 = (DrainableComboItem)Type.tryCastTo(var0.get(var2), DrainableComboItem.class);
         if (var3 != null) {
   ...
   private static final class Predicates {
      final ObjectPool category;
      final ObjectPool eval;
      final ObjectPool evalArg;
      final ObjectPool tag;
      final ObjectPool tagEval;
      final ObjectPool tagEvalArg;
      final ObjectPool type;
      final ObjectPool typeEval;
      final ObjectPool typeEvalArg;

      private Predicates() {
         this.category = new ObjectPool(() -> {
            return new ItemContainer.CategoryPredicate();
         });
         this.eval = new ObjectPool(() -> {
            return new ItemContainer.EvalPredicate();
         });
         this.evalArg = new ObjectPool(() -> {
            return new ItemContainer.EvalArgPredicate();
         });
         this.tag = new ObjectPool(() -> {
            return new ItemContainer.TagPredicate();
         });
         this.tagEval = new ObjectPool(() -> {
            return new ItemContainer.TagEvalPredicate();
         });
         this.tagEvalArg = new ObjectPool(() -> {
            return new ItemContainer.TagEvalArgPredicate();
         });
         this.type = new ObjectPool(() -> {
            return new ItemContainer.TypePredicate();
         });
         this.typeEval = new ObjectPool(() -> {
            return new ItemContainer.TypeEvalPredicate();
         });
         this.typeEvalArg = new ObjectPool(() -> {
            return new ItemContainer.TypeEvalArgPredicate();
         });
      }

      // $FF: synthetic method
      Predicates(Object var1) {
         this();
      }
   }
   ...
   private static final class TypeEvalArgPredicate implements Predicate {
      ...
      // $FF: synthetic method
      TypeEvalArgPredicate(Object var1) {
         this();
      }
   }
   ...
   private static final class TagEvalArgPredicate implements Predicate {
      ...
      // $FF: synthetic method
      TagEvalArgPredicate(Object var1) {
         this();
      }
   }
   ...
   private static final class InventoryItemListPool extends ObjectPool {
      public InventoryItemListPool() {
         super(() -> {
            return new ItemContainer.InventoryItemList();
         });
      }
   ...
   private static final class InventoryItemList extends ArrayList {
      ...
      // $FF: synthetic method
      InventoryItemList(Object var1) {
         this();
      }
   }

   private static final class Comparators {
      ObjectPool condition;
      ObjectPool eval;
      ObjectPool evalArg;

      private Comparators() {
         this.condition = new ObjectPool(() -> {
            return new ItemContainer.ConditionComparator();
         });
         this.eval = new ObjectPool(() -> {
            return new ItemContainer.EvalComparator();
         });
         this.evalArg = new ObjectPool(() -> {
            return new ItemContainer.EvalArgComparator();
         });
      }

      // $FF: synthetic method
      Comparators(Object var1) {
         this();
      }
   }
   ...
   private static final class EvalArgComparator implements Comparator {
      ...
      public int compare(InventoryItem var1, InventoryItem var2) {
         ...
         return var3.isSuccess() && !var3.isEmpty() && var3.getFirst() instanceof Double ? ((Double)var3.getFirst()).intValue() : 0;
      }

      // $FF: synthetic method
      EvalArgComparator(Object var1) {
         this();
      }
   }
   ...
   private static final class ConditionComparator implements Comparator {
      ...
      // $FF: synthetic method
      ConditionComparator(Object var1) {
         this();
      }
   }
}
